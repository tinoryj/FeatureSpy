...
{
    u_char filePathHash[SYSTEM_CIPHER_SIZE];
    cryptoObj_->generateHash((u_char*)&path[0], path.length(), filePathHash);
    memcpy(fileRecipe_.recipe.fileRecipeHead.fileNameHash, filePathHash, SYSTEM_CIPHER_SIZE);
    memcpy(fileRecipe_.recipe.keyRecipeHead.fileNameHash, filePathHash, SYSTEM_CIPHER_SIZE);

    ChunkerType_ = config.getChunkingType();

    if (ChunkerType_.compare("VariableSize") == 0) {
        int numOfMaskBits_;
        ...
        anchorValue_ = 0;
    } else if (ChunkerType_.compare("FixedSize") == 0) {
    ...
    } else if (ChunkerType_.compare("TraceFSL") == 0) {
        maxChunkSize_ = (int)config.getMaxChunkSize();
        chunkBuffer_ = new u_char[maxChunkSize_ + 6];
    } else if (ChunkerType_.compare("TraceMS") == 0) {
        maxChunkSize_ = (int)config.getMaxChunkSize();
...
bool Chunker::chunking()
{
    ...
    if (ChunkerType_.compare("FixedSize") == 0) {
        fixSizeChunking();
    }

    if (ChunkerType_.compare("TraceFSL") == 0) {
        traceDrivenChunkingFSL();
    }

    if (ChunkerType_.compare("TraceMS") == 0) {
...
void Chunker::fixSizeChunking()
{
    ...
    while (true) {
        ...
        if (totalReadSize == ReadSize_) {
            while (chunkedSize < totalReadSize) {
                ...
                memcpy(tempChunk.chunk.chunkHash, hash, SYSTEM_CIPHER_SIZE);
    ...
    second = diff / 1000000.0;
    cout << "Chunker : total read file time = " << fixed << readFileTime << " s" << endl;
    cout << "Chunker : total chunking time = " << fixed << second - (insertTime + readFileTime) << " s" << endl;
#endif
