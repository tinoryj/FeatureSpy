...
RecvDecode::RecvDecode(string fileName)
{
    ...
    if (initDownloadStatus) {
        ...
        cout << "RecvDecode : init download time = " << setprecision(8) << second << " s" << endl;
#endif
        exit(0);
    }
#if SYSTEM_BREAK_DOWN == 1
...
bool RecvDecode::recvFileHead(Recipe_t& fileRecipe, u_char* fileNameHash)
{
    ...
    request.dataSize = SYSTEM_CIPHER_SIZE;
    request.clientID = clientID_;

    int sendSize = sizeof(NetworkHeadStruct_t) + SYSTEM_CIPHER_SIZE;
    char requestBuffer[sendSize];
    char respondBuffer[1024 * 1024];
    int recvSize;

    memcpy(requestBuffer, &request, sizeof(NetworkHeadStruct_t));
    memcpy(requestBuffer + sizeof(NetworkHeadStruct_t), fileNameHash, SYSTEM_CIPHER_SIZE);
...
bool RecvDecode::processRecipe(Recipe_t& recipeHead, RecipeList_t& recipeList, u_char* fileNameHash)
{
    ...
    PRINT_BYTE_ARRAY_RECV(stderr, fileNameHash, SYSTEM_CIPHER_SIZE);
#endif
    if (!dataSecurityChannel_->send(sslConnectionData_, requestBuffer, sendSize)) {
        cerr << "RecvDecode : storage server closed" << endl;
        return false;
    ...
    if (respond.messageType == SUCCESS) {
        ...
        if (recvSize != respond.dataSize + sizeof(NetworkHeadStruct_t)) {
            ...
            u_char* requestChunkList = (u_char*)malloc(sizeof(u_char) * (SYSTEM_CIPHER_SIZE + sizeof(int)) * recipeHead.fileRecipeHead.totalChunkNumber + sizeof(NetworkHeadStruct_t));
            for (uint64_t i = 0; i < recipeHead.fileRecipeHead.totalChunkNumber; i++) {
                ...
                // memset(newRecipeEntry.chunkKeyMLE, 0, SYSTEM_CIPHER_SIZE);
                memcpy(requestChunkList + sizeof(NetworkHeadStruct_t) + i * (SYSTEM_CIPHER_SIZE + sizeof(int)), &newRecipeEntry.chunkSize, sizeof(int));
                memcpy(requestChunkList + sizeof(NetworkHeadStruct_t) + i * (SYSTEM_CIPHER_SIZE + sizeof(int)) + sizeof(int), newRecipeEntry.chunkHash, SYSTEM_CIPHER_SIZE);
            }
            free(encryptedRecipeBuffer);
            free(decryptedRecipeBuffer);
#if SYSTEM_DEBUG_FLAG == 1
            ofstream localRecipeStream;
            localRecipeStream.open("recipe.log.d", ios::out);
            ...
            uint64_t recipeListSize = recipeHead.fileRecipeHead.totalChunkNumber * (SYSTEM_CIPHER_SIZE + sizeof(int));
...
void RecvDecode::run()
{
    ...
    request.dataSize = SYSTEM_CIPHER_SIZE + 2 * sizeof(uint32_t);
    request.clientID = clientID_;
    int sendSize = sizeof(NetworkHeadStruct_t) + SYSTEM_CIPHER_SIZE;
    ...
    memcpy(requestBuffer, &request, sizeof(NetworkHeadStruct_t));
    memcpy(requestBuffer + sizeof(NetworkHeadStruct_t), fileNameHash_, SYSTEM_CIPHER_SIZE);
