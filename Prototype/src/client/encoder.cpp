    ...
    while (true) {
        ...
        if (extractMQ(tempChunk)) {
            if (tempChunk.dataType == DATA_TYPE_RECIPE) {
                ...
                if (tempChunk.chunk.logicDataSize <= INDICATOR_LENGTH) {
                    cryptoObj_->encryptWithKey(tempChunk.chunk.logicData, tempChunk.chunk.logicDataSize, tempChunk.chunk.encryptKeyFeature, ciphertext);
                } else {
                    cryptoObj_->encryptWithKey(tempChunk.chunk.logicData, INDICATOR_LENGTH, tempChunk.chunk.encryptKeyFeature, ciphertext);
                    cryptoObj_->encryptWithKey(tempChunk.chunk.logicData + INDICATOR_LENGTH, tempChunk.chunk.logicDataSize - INDICATOR_LENGTH, tempChunk.chunk.encryptKeyMLE, ciphertext + INDICATOR_LENGTH);
                }
#if SYSTEM_BREAK_DOWN == 1
