...
bool kmClient::sessionKeyUpdate()
{
    ...
    if (status != SGX_SUCCESS) {
        ...
        PRINT_BYTE_ARRAY_KM(stderr, currentSessionKey, 32);
        cerr << "KmClient : Original session key = " << endl;
        PRINT_BYTE_ARRAY_KM(stderr, currentSessionKey + 32, 16);
        cerr << "KmClient : Original mac key = " << endl;
        PRINT_BYTE_ARRAY_KM(stderr, currentSessionKey + 48, 16);
#endif
        return true;
    }
}
...
bool kmClient::init(ssl* raSecurityChannel, SSL* sslConnection)
{
    ...
    cout << "KmClient : remote attestation time = " << setprecision(8) << second << " s" << endl;
#endif
    if (enclave_trusted) {
#if SYSTEM_BREAK_DOWN == 1
        gettimeofday(&timestartkmClient, NULL);
        ...
        diff = 1000000 * (timeendKmClient.tv_sec - timestartkmClient.tv_sec) + timeendKmClient.tv_usec - timestartkmClient.tv_usec;
        second = diff / 1000000.0;
        cout << "KmClient : set key enclave global secret time = " << setprecision(8) << second << " s" << endl;
#endif
        if (status == SGX_SUCCESS) {
#if SYSTEM_DEBUG_FLAG == 1
            uint8_t ans[32];
            ...
            diff = 1000000 * (timeendKmClient.tv_sec - timestartkmClient.tv_sec) + timeendKmClient.tv_usec - timestartkmClient.tv_usec;
            second = diff / 1000000.0;
            cout << "KmClient : set key regression max counter time = " << setprecision(8) << second << " s" << endl;
#endif
            if (status == SGX_SUCCESS) {
#if SYSTEM_BREAK_DOWN == 1
                gettimeofday(&timestartkmClient, NULL);
