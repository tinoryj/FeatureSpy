...
sgx_status_t ecall_calcmac(uint8_t* src, uint32_t srcLen, uint8_t* cmac, uint8_t* chunkHashList)
{
    ...
    for (uint32_t offset = 0; offset < srcLen; offset = offset + size) {
        ...
        prefixListCounter++;
#if COUNT_INDICATOR_BY_HASH == 1
        sgx_status_t ret_status_prefix = sgx_sha256_msg(&src[offset], INDICATOR_LENGTH, &prefixHash);
        if (ret_status_prefix != SGX_SUCCESS) {
            return ret_status_prefix;
        }
        std::string prefixStr((char*)prefixHash, SYSTEM_CIPHER_SIZE);
#else
        char prefixBuffer[INDICATOR_LENGTH];
        memcpy(prefixBuffer, src + offset, INDICATOR_LENGTH);
        std::string prefixStr(prefixBuffer, INDICATOR_LENGTH);
#endif
        if (prefixList.find(prefixStr) == prefixList.end()) {
            prefixList.insert(make_pair(prefixStr, 1));
        } else {
            prefixList.at(prefixStr)++;
            if (prefixList.at(prefixStr) > PREFIX_FREQUENCY_THRESHOLD) {
                // malicious client detected
                detectFlag = true;
            }
        }
        if (prefixListCounter >= COUNT_WINDOW_SIZE) {
            prefixList.clear();
            prefixListCounter = 0;
        }
    }
