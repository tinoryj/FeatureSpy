...
ssl::ssl(string ip, int port, int scSwitch)
{
    ...
    while (true) {
        if (!IsFileUsed(CACRT) && !IsFileUsed(CLCRT) && !IsFileUsed(CLKEY) && !IsFileUsed(SECRT) && !IsFileUsed(SEKEY)) {
            cerr << "SSL : Key files not in used, Start ssl connection" << endl;
            break;
        } else {
            cerr << "SSL : Key files status " << IsFileUsed(CACRT) << IsFileUsed(CLCRT) << IsFileUsed(CLKEY) << IsFileUsed(SECRT) << IsFileUsed(SEKEY) << endl;
        }
    }
#endif
    switch (scSwitch) {
