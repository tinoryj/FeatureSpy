...
CryptoPrimitive::CryptoPrimitive()
{
    ...
    hashSize_ = SYSTEM_CIPHER_SIZE;
    //int keySize_ = SYSTEM_CIPHER_SIZE;
    ...
    chunkKeyEncryptionKey_ = (u_char*)malloc(sizeof(u_char) * SYSTEM_CIPHER_SIZE);
    memset(chunkKeyEncryptionKey_, 0, SYSTEM_CIPHER_SIZE);
...
bool CryptoPrimitive::keyExchangeCTRBaseGenerate(u_char* nonce, uint32_t counter, uint32_t generateNumber, u_char* key, u_char* iv, u_char* ctrBaseBuffer)
{
    ...
    for (int i = 0; i < generateNumber / 2; i++) {
        ...
        memcpy(currentKeyBase + sizeof(uint32_t), nonce, CRYPTO_BLOCK_SIZE - sizeof(uint32_t));
        currentCounterTemp++;
        memcpy(currentKeyBase + CRYPTO_BLOCK_SIZE, &currentCounterTemp, sizeof(uint32_t));
        memcpy(currentKeyBase + CRYPTO_BLOCK_SIZE + sizeof(uint32_t), nonce, CRYPTO_BLOCK_SIZE - sizeof(uint32_t));
        currentCounterTemp++;
...
bool CryptoPrimitive::cmac128(u_char* hashList, uint32_t chunkNumber, u_char* mac, u_char* key, int keyLen)
{
    ...
    for (int i = 0; i < chunkNumber; i++) {
        CMAC_Update(cmacctx_, (void*)(hashList + i * SYSTEM_CIPHER_SIZE), SYSTEM_CIPHER_SIZE);
    }
    size_t maclen;
