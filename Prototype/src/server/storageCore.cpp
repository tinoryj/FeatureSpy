...
bool StorageCore::storeChunks(NetworkHeadStruct_t& networkHead, char* data)
{
    ...
    for (int i = 0; i < chunkNumber; i++) {
        ...
        string originHash(data + readSize, SYSTEM_CIPHER_SIZE);
        readSize += SYSTEM_CIPHER_SIZE;
...
{
    string recipeName;
    string DBKey(fileNameHash, SYSTEM_CIPHER_SIZE);
...
bool StorageCore::storeRecipes(char* fileNameHash, u_char* recipeContent, uint64_t recipeSize)
{
    ...
    if (fileName2metaDB.query(DBKey, recipeName)) {
        ...
        char recipeNameBuffer[SYSTEM_CIPHER_SIZE * 2 + 1];
        ...
        string recipeNameNew(recipeNameBuffer, SYSTEM_CIPHER_SIZE * 2);
...
bool StorageCore::restoreRecipeAndChunk(char* recipeList, uint32_t startID, uint32_t endID, char* restoredChunkList, int& restoredChunkNumber, int& restoredChunkSize)
{
    ...
    for (int i = 0; i < restoredChunkNumber; i++) {
        int chunkSize = 0;
        memcpy(&chunkSize, recipeList + i * (SYSTEM_CIPHER_SIZE + sizeof(int)), sizeof(int));
        string chunkHash(recipeList + i * (SYSTEM_CIPHER_SIZE + sizeof(int)) + sizeof(int), SYSTEM_CIPHER_SIZE);
...
bool StorageCore::storeChunk(std::string chunkHash, char* chunkData, int chunkSize)
{
    ...
    if (chunkHash.size() != SYSTEM_CIPHER_SIZE) {
...
#endif
#if SYSTEM_LOG_FLAG == 1
    cerr << "ContainerManager : save " << fixed << this->used_ << " bytes to file system" << endl;
#endif
    containerOut.close();
    return true;
}